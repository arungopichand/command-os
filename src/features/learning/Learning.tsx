...
export function Learning() {
  ...
  const [vault, setVault] = useState<{id: string, type: 'book'|'course'|'skill', title: string, progress: string, link: string}[]>([]);
  ...
  const addResource = (e: React.FormEvent) => {
    e.preventDefault();
    if (!title) return;
    setVault([{ id: generateUUID(), type, title, progress, link }, ...vault]);
    setTitle('');
    setProgress('');
    setLink('');
  };
